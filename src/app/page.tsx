    ...
    <main>
      <section className="relative overflow-hidden bg-ink px-5 py-6 text-white sm:px-8 lg:px-12">
        ...
        <nav className="mx-auto flex max-w-7xl items-center justify-between border-b border-white/10 pb-5">
          ...
          <a
            href="#dashboard"
            className="font-serif inline-flex items-center gap-2 rounded-md border border-white/16 px-4 py-2 text-sm text-white/80 transition hover:border-white/40 hover:text-white"
          >
            Open Workbench
        ...
        <div className="mx-auto grid max-w-7xl gap-10 pb-14 pt-14 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
          <div>
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 font-mono text-white/62">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#dashboard"
                className=" font-mono inline-flex items-center gap-2 rounded-md bg-white px-5 py-3 text-sm font-semibold text-ink transition hover:bg-white/88"
              >
                View Live Prototype
              ...
              <a
                href="#architecture"
                className="font-mono inline-flex items-center gap-2 rounded-md border border-white/16 px-5 py-3 text-sm font-semibold text-white/80 transition hover:border-white/40 hover:text-white"
              >
                See Pipeline
                <ArrowRight className="h-4 w-4" aria-hidden />
              </a>
          ...
          <div className="border-l border-white/12 pl-6">
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="font-mono rounded-md border border-white/12 bg-white/[0.04] p-4">
            ...
            <div className="mt-4 rounded-md border border-white/12 bg-white/[0.04] p-5">
              ...
              <p className="mt-3 text-2xl font-serif font-semibold text-white">Synthesis beats raw feeds.</p>
              <p className="mt-3 text-sm leading-6 text-white/55 font-mono">
      ...
      <section id="architecture" className="bg-paper px-5 py-10 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.18em]">Serverless Pipeline</p>
              ...
              <p className="mt-5 text-base leading-7 text-ink/62 font-mono">
            ...
            <div className="grid gap-3 md:grid-cols-2 font-mono">
              {flow.map((step, index) => (
                <div key={step.title} className="rounded-md border border-line bg-paper p-5">
                  <div className="flex items-center justify-between gap-3">
                    <step.icon className="h-5 w-5 text-blue" aria-hidden />
                    <span className="font-mono text-xs text-ink/35">0{index + 1}</span>
                  </div>
                  <h3 className="mt-5 text-xl font-semibold text-[#11110f]">{step.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#11110f]">{step.text}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="border-y border-line bg-paper px-5 py-8 sm:px-8 lg:px-12">
        <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-3 font-mono">
          {[
            ...
            ["Compliance posture", "Public-source provenance and pull history are first-class records."]
              ...
              <div>
                <h3 className="font-semibold text-[#11110f]">{title}</h3>
                <p className="mt-1 text-sm leading-6 text-[#11110f]">{text}</p>
              </div>
            </div>
          ))}
        </div>
